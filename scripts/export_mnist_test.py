...
class ExportMnistEmbeddingTest(tf.test.TestCase):
...
#         embedding_file=self._embedding_file_path,
# #        export_path=self.get_temp_dir(),
#         export_path="/Users/smap3/tensorflow_hub/module_cache/sample",
#         parse_line_fn=export.parse_line,
#         num_oov_buckets=1,
#         preprocess_text=True)
#     with tf.Graph().as_default():
# #      hub_module = hub.Module(self.get_temp_dir())
#       hub_module = hub.Module("/Users/smap3/tensorflow_hub/module_cache/sample")
...
#             rtol=0.02)
    ...
    def testExportMnistPredictModule(self):
        mnist = tf.keras.datasets.mnist

        (x_train, y_train), (x_test, y_test) = mnist.load_data()
        x_train, x_test = x_train / 255.0, x_test / 255.0

        export_mnist.export(export_path=self.get_temp_dir())

        with tf.Graph().as_default():
            hub_module = hub.Module(self.get_temp_dir())
            image_tensor = tf.constant(x_test[0][np.newaxis, :, :])
            predict = hub_module(image_tensor)
            with tf.Session() as session:
                session.run(tf.tables_initializer())
                session.run(tf.global_variables_initializer())
                self.assertAllClose(session.run(predict),[7])
